...
KeyDialog::KeyDialog(QWidget *parent) :
    ...
    _editTtl->setPlaceholderText("空不修改，负数为永久...");
...
void KeyDialog::onOK() {
    ...
    if(_strKey.isEmpty()) {
        QMessageBox::critical(this, "错误", "键不可为空");
        return;
    }
    ...
    for(int i = 0; i < _strbTtl.size(); ++i) {
        ...
        if(c < '0' || c > '9') {
            if(c != '-') {
                QMessageBox::critical(this, "错误", "数据错误，键超时时间不是有效数值");
                return;
            }
        }
    }
    ...
    if(!_flag) {
        ...
        if(_strValue.isEmpty()) {
            QMessageBox::critical(this, "错误", "值不可为空");
            return;
        }
