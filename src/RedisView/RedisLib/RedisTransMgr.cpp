...
void RedisTransMgr::open()
{
    open(_hostAddress, _nPort);
}

void RedisTransMgr::open(const QString &hostAddress, quint16 port)
{
    open(QHostAddress(hostAddress), port);
}

void RedisTransMgr::open(const QHostAddress & hostAddress, quint16 port)
{
    _hostAddress = hostAddress;
    _nPort = port;
    _socket->connectToHost(hostAddress, port);
    _socket->waitForConnected(3000); // 默认超时3秒
}
