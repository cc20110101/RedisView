...
void RedisClient::open(const QString &hostAddress,quint16 port) {
    _redisTransMgr->open(hostAddress, port);
}
...
bool RedisClient::isCluster(bool &value) { 
    ...
    if(_cmdReault[0] == '-') {
        ...
        QByteArray sValue;
        if(parseRespBulkString(_cmdReault,sValue, _iRet)) {
            if(_iRet == -1) {
                _bRet = false;
...
bool RedisClient::decrby(const QString & key, int interval, qlonglong & llRet) {
    ...
    _cmd.append(key);
    _cmd.append(" ");
    _cmd.append(QString::number(interval));
...
bool RedisClient::hincrby(const QString & key, const QString & field, int increment, qlonglong & llRet) {
    ...
    _cmd.append(field);
    _cmd.append(" ");
    _cmd.append(QString::number(increment));
...
bool RedisClient::expire(const QString & key, uint seconds) {
    ...
    _cmd.append(key);
    _cmd.append(" ");
    _cmd.append(QString::number(seconds));
...
bool RedisClient::pexpire(const QString & key, uint mseconds) {
    ...
    _cmd.append(key);
    _cmd.append(" ");
    _cmd.append(QString::number(mseconds));
...
bool RedisClient::pexpireat(const QString & key, uint mstimestamp) {
    ...
    _cmd.clear();
    _cmd.append("PEXPIREAT ");
    _cmd.append(key);
    _cmd.append(" ");
    _cmd.append(QString::number(mstimestamp));
...
bool RedisClient::expireat(const QString & key, uint timestamp) {
    ...
    _cmd.clear();
    _cmd.append("EXPIREAT ");
    _cmd.append(key);
    _cmd.append(" ");
    _cmd.append(QString::number(timestamp));
...
bool RedisClient::getrange(const QString & key, int start, int stop, QByteArray & value) {
    ...
    _cmd.clear();
    _cmd.append("GETRANGE ");
    _cmd.append(key);
    _cmd.append(" ");
    _cmd.append(QString::number(start));
    _cmd.append(" ");
    _cmd.append(QString::number(stop));
...
bool RedisClient::blpop(const QString & key, QString & outkey, QByteArray & value, int timeout) {
    ...
    _cmd.append(key);
    _cmd.append(" ");
    _cmd.append(QString::number(timeout));
    ...
    if(_cmdReault[0] == '-') {
        ...
        _vListResult.clear();
        if(parseRespSingelArray(_cmdReault,_vListResult, _iRet)) {
            if(_iRet == -1) {
                _bRet = false;
                _sErrorInfo = "value is nil";
            } else if(_iRet >= 2){
...
bool RedisClient::lrem(const QString & key, int count, const QString & value, qlonglong & llRet) {
    ...
    _cmd.append(key);
    _cmd.append(" ");
    _cmd.append(QString::number(count));
    _cmd.append(" ");
    _cmd.append(value);
...
bool RedisClient::move(const QString & key, int database) {
    ...
    _cmd.append(key);
    _cmd.append(" ");
    _cmd.append(QString::number(database));
...
bool RedisClient::rpoplpush(const QString & source, const QString & destination, QByteArray & value) {
    ...
    _cmd.clear();
    _cmd.append("RPOPLPUSH ");
    _cmd.append(source);
    _cmd.append(" ");
    _cmd.append(destination);
...
bool RedisClient::zadd(const QString &key, const QString &member, const double &score, qlonglong & llRet) {
    ...
    _cmd.append(key);
    _cmd.append(" ");
    _cmd.append(QString::number(score));
    _cmd.append(" ");
    _cmd.append(member);
...
bool RedisClient::setrange(const QString & key, int offset, const QString & value, qlonglong & llRet) {
    ...
    _cmd.clear();
    _cmd.append("SETRANGE ");
    _cmd.append(key);
    _cmd.append(" ");
    _cmd.append(QString::number(offset));
    _cmd.append(" ");
    _cmd.append(value);
...
bool RedisClient::migrate(const QString & host, int port, const QString & key,
    ...
    _cmd.append(host);
    _cmd.append(" ");
    _cmd.append(QString::number(port));
    _cmd.append(" ");
    _cmd.append(key);
    _cmd.append(" ");
    _cmd.append(QString::number(database));
...
bool RedisClient::hgetall(const QString & key, QMap<QString, QString> & vKeyPairs, int & iRet) {
    ...
    if(_cmdReault[0] == '-') {
        ...
        _vListResult.clear();
        if(parseRespSingelArray(_cmdReault,_vListResult, iRet)) {
            if(iRet <= 0) {
                _bRet = false;
                _sErrorInfo = "value is nil";
            } else if(iRet >= 2){
...
bool RedisClient::scan(const QString &pattern, RespType &value, qulonglong cursor, qulonglong count) {
    ...
    _cmd.clear();
    _cmd.append("SCAN ");
    _cmd.append(QString::number(cursor));
    if(!pattern.isEmpty()) {
        _cmd.append(" MATCH ");
        _cmd.append(pattern);
    }
