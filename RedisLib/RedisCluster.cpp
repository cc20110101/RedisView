...
QList<QByteArray> RedisCluster::clusterCommand(QString &str) {
    ...
    if(str.startsWith("randomkey",Qt::CaseInsensitive) ||
        ...
        result << "-command is not supported now\r\n";
        return result;
    }
...
bool RedisCluster::select(const int &index) {
    if(_isClusterMode && index == 0) {
        ...
        return _redisClient->select(index);
    } else {
...
QList<QByteArray> RedisCluster::command(QString &str, const QString &split) {
    if(split.isEmpty()) {
        if(_isClusterMode)
...
QByteArray RedisCluster::singleCommand(QString &str)
{
    str = str.trimmed();
    //str.replace(QRegExp("[\\s]+"), " ");
...
RedisClient * RedisCluster::getClient() {
    if(_isClusterMode) {
        if(_vClusterMasterClients.size() > 0) {
            _iClientIndex = 0;
            return _vClusterMasterClients[0]._client;
        } else
            return nullptr;
    } else {
        _iClientIndex = -1;
...
void RedisCluster::close() {
    ...
    if(_isClusterMode) {
        for(int i = 0; i < _vClusterMasterClients.size(); ++i) {
            if(_vClusterMasterClients[i]._client) {
                _vClusterMasterClients[i]._client->close();
                delete _vClusterMasterClients[i]._client;
                _vClusterMasterClients[i]._client = nullptr;
            }
        }
    } else {
        if(_redisClient) {
...
QList<ClusterClient> RedisCluster::getClients() const
{
    if(_isClusterMode) {
...
bool RedisCluster::openSingel(const QString &hostAddress, const QString passwd) {
...
bool RedisCluster::openCluster(const QString &hostAddress, const QString passwd) {
    ...
    for(int i = 0; i < hostList.size(); /*++i*/) {
        ...
        _redisClient->open(clusterClient._host, clusterClient._port);
        if(_redisClient->isOpen()) {
            if(!clusterClient._passwd.isEmpty()) {
    ...
    for(int m = 0; m < _vClusterClients.size(); ++m) {
        if(_vClusterClients[m]._master) {
            _redisClient = new RedisClient();
            _redisClient->open(_vClusterClients[m]._host,
