    ...
    QDialog(parent)
{
    ...
    setWindowTitle("Redis连接");
    _redisClient = nullptr;

    _labelCreate = new QLabel("<a style='color:#1F90FE;' href=create><u>创建</u>");
    _labelEdit = new QLabel("<a style='color:#1F90FE;' href=edit><u>编辑</u>");
    _labelDelte = new QLabel("<a style='color:#1F90FE;' href=delete><u>删除</u>");
    ...
    _btConnect = new QPushButton("连接");
    _btCancel = new QPushButton("取消");
...
void LoginDialog::onLink(const QString &url) {
    ...
    if(url == "create") {
        ...
        _treeWidgetItem = _treeWidget->currentItem();
        if(_treeWidgetItem) {
            if(QMessageBox::Yes != QMessageBox::question(this, "确认", "是否要删除此登入信息？"))
                return;
            for(int i = 0; i < _vClientInfo.size(); ++i) {
...
void LoginDialog::onOK() {
    ...
    if(_treeWidgetItem) {
        ...
        if(_redisClient->openCluster(_treeWidgetItem->text(1),
            ...
            QMessageBox::critical(this, "连接错误", _redisClient->getErrorInfo());
        }
    }
