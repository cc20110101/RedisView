...
void LoginSet::getClientInfo(ClientInfo & clientInfo)
{
...
void LoginSet::setClientInfo(ClientInfo & clientInfo)
{
...
void LoginSet::onOK() {
    ...
    if(_clientInfo._name.isEmpty()) {
        QMessageBox::critical(this, "错误", "数据库连接标签名称不可为空!");
        _le_name->setFocus();
        return;
    }
    ...
    if(_clientInfo._addr.isEmpty()) {
        QMessageBox::critical(this, "错误", "数据库连接串不可为空!");
        _le_addr->setFocus();
        return;
    }
